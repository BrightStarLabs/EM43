#!/usr/bin/env python3
"""
EM43 Unified Demo - Complete Training, Evaluation & Inference Pipeline
=====================================================================

A unified demo script for the refactored EM43 system featuring:
- Automatic mode detection (1-input vs 2-input)
- Interactive task selection
- Stage-based execution (train → evaluate → infer)
- Config integration with command-line overrides
- Support for all 17+ tasks

Usage Examples:
    # Basic training and evaluation
    python demo.py --task 1 --stage train      # Train multiply-by-2 task
    python demo.py --task 20 --stage evaluate  # Evaluate summation task
    
    # Interactive mode
    python demo.py --interactive               # Browse and select tasks
    
    # Custom parameters  
    python demo.py --task 1 --pop-size 1000 --generations 50
    
    # Full pipeline
    python demo.py --task 2 --stage all       # Train → Evaluate → Infer
    
    # Inference only
    python demo.py --task 1 --stage infer --inputs "1,2,3,4,5"
...
def stage_evaluate(model: EM43Wrapper, args) -> bool:
    ...
    try:
        ...
        results = model.evaluate(verbose=not args.quiet, plot=not args.no_plot)
        
        if not args.quiet and results.get('targets') is not None:
            print(f"\n📈 Evaluation Summary:")
