...
# ───────────────── Vectorised fitness for two inputs ────────────────────
@nb.njit(parallel=True, fastmath=True, cache=True, )
def fitness_population_two_inputs(rules: np.ndarray, progs: np.ndarray, inputs_a: np.ndarray, inputs_b: np.ndarray, target_out: np.ndarray, window: int, max_steps: int, halt_th: float, lambda_p: float) -> np.ndarray:
    """Compute fitness for every genome in parallel with two inputs.
    rules     : (P,64)  uint8
    progs     : (P,L)   uint8  
    inputs_a  : (B,) int64
    inputs_b  : (B,) int64
    ...
    fitness = np.empty(P, dtype=np.float32)
    for i in nb.prange(P):
        outs = _simulate_two_inputs(rules[i], progs[i], inputs_a, inputs_b, window,
                                   max_steps, halt_th)
        avg_err = np.abs(outs - target_out).mean()
        sparsity = np.count_nonzero(progs[i]) / progs.shape[1]
        fitness[i] = -avg_err - lambda_p * sparsity
