...
if stage == "train":
    ...
    use_random_search = args.enable_rs.lower() == 'true'
    
    if use_random_search:
        print("Starting EM-4/3 Random Search with parameters:")
        for k, v in vars(args).items():
            print(f"\t{k}: {v}")
        print("------------------------------\n")

        rs = RandomSearch(args)
        best_rule, best_prog, best_fitness = rs.run()
        
        print("\n------------------------------")    
        print(f"Elapsed {time.time()-t0:.1f}s")
        print(f"Best fitness (Random Search): {best_fitness:.3f}")
        ...
        print("\n------------------------------")    
        print(f"Elapsed {time.time()-t0:.1f}s")
        print(f"Best fitness (GA): {best_fitness:.3f}")
        print(f"Best rule: {best_rule}")
        print(f"Best program: {best_prog}")
        print("\n------------------------------")
